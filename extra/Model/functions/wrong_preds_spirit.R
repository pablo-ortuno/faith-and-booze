...
wrong_preds_spirit <- function(year_to_predict)
  
{
  ...
  my_data <- predict(model_election, model_data_test, type = "prob") %>%
    ...
    filter(pred_for_plot %in% c('actually democrat', 'actually republican')) %>% 
    rename( Wrong_Predictions = pred_for_plot ) %>% 
    count( Wrong_Predictions )
  
  my_data <- my_data%>% 
    add_row('Wrong_Predictions' = 'Total', n = sum(my_data$n) )
  
  names(my_data)[names(my_data) == "Wrong_Predictions"] <- paste( 'Wrong Predictions', year_to_predict)
